...
?>
                        <?php  // Réinitialisation des variables pour chaque véhicule
                     
                        // Parcours des données de suivi pour le véhicule en cours
                        foreach($suivi_data as $rows){
                        if ($rows['matricule_v'] == $row['matricule_v']) {
                         // Calcul de vidange et de chaine pour ce véhicule
                         $km_ret = $rows['km_ret'];
                        $km_dep = $rows['km_dep'];

                        $vidange += ($km_ret - $km_dep);
                        $chaine += ($km_ret - $km_dep);
                        }
                            }

                        // Affichage des valeurs de vidange et de chaine pour débogage
                        echo 'vidange : ' . $vidange . '<br>';
                        ...
                        $display='inline-block;'; //Afficher le boutton
                                ...
                                $plaquette = ($km_actuel / $seuil_plaquette) * 100;
                                if($plaquette > 95){
...
if (isset($_POST['di'])) {
    ...
    try {
        ...
        $stmt_km = $conn->prepare("SELECT km_actuel,km_init FROM véhicule WHERE matricule_v = :matricule_v");
        ...
        $stmt_update = $conn->prepare("UPDATE véhicule SET km_init = :km_init, km_actuel = :km_actuel WHERE matricule_v = :matricule_v");
